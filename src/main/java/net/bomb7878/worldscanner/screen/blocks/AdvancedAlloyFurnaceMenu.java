package net.bomb7878.worldscanner.screen.blocks;
...
public class AdvancedAlloyFurnaceMenu extends AbstractContainerMenu {
    public final AdvancedAlloyFurnaceBlockEntity blockEntity;
    private final Level level;
    private final ContainerData data;
    private FluidStack fluidStack;

    public AdvancedAlloyFurnaceMenu(int id, Inventory inventory, FriendlyByteBuf buf) {
        this(id, inventory, inventory.player.level.getBlockEntity(buf.readBlockPos()), new SimpleContainerData(2));
    }

    public AdvancedAlloyFurnaceMenu(int id, Inventory inventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.ADVANCED_ALLOY_FURNACE_MENU.get(), id);
        checkContainerSize(inventory, 5);
        checkContainerDataCount(data, 2);
        this.blockEntity = (AdvancedAlloyFurnaceBlockEntity) entity;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                player, ModBlocks.ADVANCED_ALLOY_FURNACE.get());
    }
