package net.bomb7878.worldscanner.datagen;

import net.bomb7878.worldscanner.WorldScanner;
import net.bomb7878.worldscanner.blocks.custom.AdvancedAlloyFurnaceBlock;
import net.bomb7878.worldscanner.blocks.custom.AlloyFurnaceBlock;
import net.bomb7878.worldscanner.blocks.ModBlocks;
import net.minecraft.core.Direction;
import net.minecraft.data.PackOutput;
import net.minecraft.world.level.block.Block;
import net.minecraftforge.client.model.generators.BlockStateProvider;
import net.minecraftforge.client.model.generators.ConfiguredModel;
import net.minecraftforge.client.model.generators.ModelFile;
import net.minecraftforge.common.data.ExistingFileHelper;
import net.minecraftforge.registries.RegistryObject;

public class ModBlockStateProvider extends BlockStateProvider {
    public ModBlockStateProvider(PackOutput output, ExistingFileHelper exFileHelper) {
        super(output, WorldScanner.MOD_ID, exFileHelper);
    }

    @Override
    protected void registerStatesAndModels() {
        blockWithItem(ModBlocks.BLOCK_OF_BRONZE);
        blockWithItem(ModBlocks.BLOCK_OF_STEEL);
        blockWithItem(ModBlocks.BLOCK_OF_DARK_DIAMOND);
        blockWithItem(ModBlocks.BLOCK_OF_TIN);
        blockWithItem(ModBlocks.BLOCK_OF_RAW_TIN);
        blockWithItem(ModBlocks.DARK_DIAMOND_ORE);
        blockWithItem(ModBlocks.TIN_ORE);
        blockWithItem(ModBlocks.BLOCK_OF_DARK_STEEL);
        blockWithItem(ModBlocks.BLOCK_OF_ENDER_STEEL);

        registerAlloyFurnace(ModBlocks.ALLOY_FURNACE.get(),"block/alloy_furnace_inactive", "block/alloy_furnace_active");
        registerAlloyFurnace(ModBlocks.ADVANCED_ALLOY_FURNACE.get(),"block/advanced_alloy_furnace_inactive", "block/advanced_alloy_furnace_active");
    }

    private void registerAlloyFurnace(Block alloyFurnaceBlock,String inactivePNGLocation, String activePNGLocation) {
        // Создаем модели для разных состояний
        ModelFile inactiveModel = models().getExistingFile(
                modLoc(inactivePNGLocation));
        ModelFile activeModel = models().getExistingFile(
                modLoc(activePNGLocation));

        // Генерируем все варианты состояний
        getVariantBuilder(alloyFurnaceBlock)
                .forAllStates(state -> {
                    Direction facing = null;
                    boolean active = false;
                    if(alloyFurnaceBlock instanceof AlloyFurnaceBlock) {
                        facing = state.getValue(AlloyFurnaceBlock.FACING);
                        active = state.getValue(AlloyFurnaceBlock.ACTIVE);
                    } else if (alloyFurnaceBlock instanceof AdvancedAlloyFurnaceBlock) {
                        facing = state.getValue(AdvancedAlloyFurnaceBlock.FACING);
                        active = state.getValue(AdvancedAlloyFurnaceBlock.ACTIVE);
                    }

                    int yRot = switch (facing) {
                        case EAST -> 90;
                        case SOUTH -> 180;
                        case WEST -> 270;
                        default -> 0; // NORTH
                    };

                    ModelFile model = active ? activeModel : inactiveModel;
                    return ConfiguredModel.builder()
                            .modelFile(model)
                            .rotationY(yRot)
                            .build();
                });
    }

    private void blockWithItem(RegistryObject<Block> block) {
        simpleBlockWithItem(block.get(), cubeAll(block.get()));
    }
}
